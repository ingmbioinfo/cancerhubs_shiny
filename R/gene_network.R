...
create_network <- function(data, original, cancer_type, int_type, gene, include_mutated = TRUE, crosses = FALSE ) {
  ...
  # Find the row corresponding to the gene
    ...
    if (length(interactors) > 50) {
    
    tumor_data <- original[[cancer_type]][["All_Genes"]]
    filtered_data <- tumor_data[tumor_data$gene_list %in% interactors, ]
    
    # Step 2: Sort the filtered data by the "Network_Score" column in descending order
    sorted_data <- filtered_data[order(-filtered_data$network_score), ]
    
    # Step 3: Extract the top 50 interactors
    top_50 <- sorted_data[1:50, ]
    
    # Step 4: Match the top 50 interactors back to the original interactors' structure
    ...
    #Sort the filtered data by the "Network_Score" column in descending order
    sorted_data <- filtered_data[order(-filtered_data$network_score), ]
  ...
  for (i in seq_along(interactors)) {
    ...
    print(paste("Interactor:", interactor_i))
 ...
 plot(g, vertex.size = 10, vertex.label.cex = 1,vertex.color =  ifelse(V(g)$name == gene, "pink", "#83C9C8"),
      vertex.frame.color = ifelse(V(g)$name == gene, "pink", "#83C9C8"),
      vertex.label.color= "black",vertex.label.font=2, main = paste("Top50 Interactors of", gene))

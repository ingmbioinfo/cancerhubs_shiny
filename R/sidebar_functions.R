    ...
    div(class = "sidebar",
        conditionalPanel(
          ...
          p(style = "white-space: pre-wrap",
            "In this first section you can explore the original tables of our anslysis.\nThe main one, 'All Genes' is where all relevant genes selected for their muational status and presence in PRECOG database are listed.\nThe other three subcategories are derived from that one.\n In each table genes are ranked on the basis of their network score, which accounts for their centrality and significance with respect to their interactors."),
        ...
        conditionalPanel(
          ...
          h4("Parameters selection"),
          p('This section allows you to display in an interactive way the network of interactions that exist among the highest network-scoring genes in your interest tumor. The colors can be set to represent the network score or the precog metaZ score.'),
        ...
        conditionalPanel(
          ...
          h4("Parameters selection"), 
          p('In this panel you can search your interest gene and see up to 50 of its interactors which are selected, if possible, on the basis of their Network Score if present in this dataset.\nThis is not always feasible because not all the interactors are present in the relevant genes identified by Cancerhubs or they are not present in the current category (specified upon selection), and for this reason, they do not have the score.',
            style = "white-space: pre-wrap;"),
          br(),
          p(tags$span(style = "font-weight: bold; white-space: pre-wrap;","If genes with no network score are present in the top 50, their presence does NOT mean they are more meaningful than others that are not present.\n(The complete interactors tables are available for download below.)")),
          br(),
          p('The interactors in "All Genes" and "Only Mutated" are the ones reported in the intial tables. Instead, when selecting a category that includes precog genes, also its interactors will be precog by default.',
            style = "white-space: pre-wrap;"),
          br(),
          selectInput("network_tumor", "Select Tumor:", choices = names(data)),

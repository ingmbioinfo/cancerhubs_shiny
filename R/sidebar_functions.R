  ...
  sidebarPanel(
    div(class = "sidebar",
        ...
        conditionalPanel(
          ...
          h4("Parameters selection"), 
          p('In this panel you can search your interest gene and see up to 50 of its interactors selected, if possible, on the basis of their Network Score.\nThis is not always feasible because not all the interactors are present in the relevant genes identified by Cancerhubs, and so, they do not have the score.\nIf genes with no network score are present in the top 50 their presence does NOT mean they are more meaningful than others that are not present.',
            style = "white-space: pre-wrap;"),
          br(),
          p('Upon selection you can also visualize the totality of interactions among the shown genes and select for the visualization of only muated interactors.'),
          br(),
          selectInput("network_tumor", "Select Tumor:", choices = names(data)),
          selectInput("data_type_precog", "Select Dataset Type:", choices = c("ALL","Only MUTATED (Not Precog)", "PRECOG")),
          ...
          br(),
          h4("Downloads"), 
          p("Here you can download the network plot and the complete table of interaction of your interest gene.\nBelow that, you'll also find the WHOLE dataset of interactions for all the genes that match your paramters selection.", style = "white-space: pre-wrap;"), # Add the formal text
          br(),
